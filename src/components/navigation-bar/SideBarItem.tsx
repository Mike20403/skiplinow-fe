...
export const SideBarItem = ({ id, icon, title, active, onClick }: SideBarItemProps) => {
  return (
    <>
      ...
      >
        <div className="flex flex-row items-center justify-center w-10 h-10 max-w-[2.5rem] rounded-md">{icon}</div>
        <h1 className="hidden md:block ml-4 text-sm font-semibold ">{title}</h1>
      </div>
      <hr />
    </>
  );
