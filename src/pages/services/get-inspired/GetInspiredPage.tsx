...
import {
  ideaGeneratorValidationSchema, topicGeneratorValidationSchema,
...
export const GetInpsiredPage = (props:GetInpsiredPageProps) => {
  ...
  const handleEntityClick = (idea:string) => {
    ...
    <>
      ...
      {idea !== UNKNOWN  ? (
        <>
          ...
          <h1 className={`${!(generatedCaptions.length) ? 'hidden' : ''} text-lg font-bold mt-5`}>Captions generated for
            you:</h1>
          <div className="generated-section flex flex-col gap-6 mt-5">
            {
              generatedCaptions.length > 0 && (
                generatedCaptions.map((caption, index) => (
                  <EntityCard caption={caption} idea={idea} onEntityClick={() => {
                  }} key={index} />
                )))
            }
          </div>
        ...
        <>
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {generatedIdeas.map((idea, index) => (
                    <Link to={`.?idea=${idea}`}>
                      <EntityCard key={index} caption={idea} disabledAction={true} onEntityClick={handleEntityClick} />
                  </Link>
                ))}
              </div>
            </>
          ...
          : (
            <>
              <p className="text-lg">Stick staring at a blank page? Tell us what topic you have in mind and Whale AI will
                generate a list of post ides and captions for you</p>
